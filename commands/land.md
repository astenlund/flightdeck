...
2. **Verify end-to-end.** Drive the affected flow in the running app or tool and observe the behavior (the `verify` skill's discipline: exercise the change, don't just trust tests or typecheck). Skip with a one-line note when the session's diff has no runtime surface to drive (docs-only or test-only changes).
