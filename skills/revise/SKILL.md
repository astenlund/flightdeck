...
Then run any artifact-specific retrospective items the artifact file adds. If changes are warranted, route each one: loop-mechanics improvements belong in this SKILL.md (shared by all artifact types); dimension or artifact-specific improvements belong in the artifact parameter file (`code.md` / `plan.md` / `spec.md`). Edit the files in the flightdeck plugin repo clone, not the installed cache. Show the diff, explain why, apply with the user's approval, and commit in the clone; pushing is the user's call.
