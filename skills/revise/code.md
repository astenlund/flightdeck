...
- **Post-fix steps**: rebuild (build only, no tests) to confirm nothing is broken. If the user explicitly set the scope to `staged` (i.e., they typed `/nightshift:revise-code staged`), stage the fixes so `git diff --cached` includes them in the next iteration. Do not stage otherwise — agents read files directly and don't need staging to see fixes. Commit each iteration's verified fixes at the iteration boundary when the session's and project's commit conventions permit unprompted commits: as `git commit --fixup=<sha>` of the corrected commit when the scope is a commit range, as a standalone commit otherwise. This keeps fixes durable across session loss and preserves the clean-tree invariant (patch == live files) at every boundary; a later iteration overturning a committed fix just adds another fixup, which autosquash folds. Where commits require explicit direction, leave fixes uncommitted and regenerate the patch against the working tree (`git diff <base>`) instead. Never commit mid-batch -- fixes are evaluated, applied, and verified green only after every agent in the iteration has returned.
